...
function openProgram(id){state.activeProgramId=id;state.schedulerExpanded=true;activeView='scheduler';state.activeTab='scheduler';let p=state.programs.find(x=>x.id===id);if(p){p.group=normGroup(p.group);ensureGroupDefaults(p.group)} if (window.innerWidth > 1024) document.getElementById('sidebar')?.classList.add('collapsed'); save()}
